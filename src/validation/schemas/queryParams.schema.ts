...
export const queryParamsSchema = z
	.object({
		...
		sortBy: z.enum(allowedSortFields, {
			message: `Sort by must be one of: ${allowedSortFields.join(", ")}`,
		}).optional(),
		order: z.enum(allowedOrderValues, {
		...
		language: z
			...
			.optional(),
		lat: optionalCoercedNumber("Latitude must be a number")
			.refine((value) => value === undefined || (value >= -90 && value <= 90), {
				message: "Latitude must be between -90 and 90",
			}),
		lng: optionalCoercedNumber("Longitude must be a number")
			.refine(
		...
		radius: optionalCoercedNumber("Radius must be a number")
			.refine((value) => value === undefined || value > 0, {
