...
export class NotificationRepository {
    ...
    async create(
        ...
        if (!data.userId && !data.guestSessionId) {
            throw new Error("Cannot create notification: Must provide either userId or guestSessionId");
        }
     ...
     */
    async markAllAsRead(identifier: { userId?: string; guestSessionId?: string }): Promise<number> {
        const userCondition = identifier.userId 
            ? eq(notifications.userId, identifier.userId)
            : eq(notifications.guestSessionId, identifier.guestSessionId as string);
