import { eq, avg, count as drizzleCount } from "drizzle-orm";
import { db } from "../../db";
...
export class VolunteerRepository {
   ...
   */
  async update(id: number, data: UpdateVolunteerDTO): Promise<Volunteer | undefined> {
  ...
  async findRatingsById(id: number): Promise<{
    ratings: { id: number; stars: number; comment: string | null; createdAt: Date; writtenByUserId: string }[];
    ...
    const averageStars =
      allRatings.reduce((sum: number, r: { stars: number }) => sum + r.stars, 0) /
      allRatings.length;
  ...
  async count(): Promise<number> {
    const [{ value }] = await db.select({ value: drizzleCount() }).from(volunteers);
    return value;
  }
}
