...
export class HelpRequestRepository
    ...
    async findPaginatedWithDetails(
        ...
        const primarySort =
            order === "ASC"
                ? asc(helpRequests[sortBy])
                : desc(helpRequests[sortBy]);
        ...
        // In-memory skill match score calculation and sorting if requested
        let data = rows.map(({helpRequest, requestDetails}) => {
            const skillScore = calculateSkillMachScore(
                requestedSkills, //de la voluntar
        ...
        //remove `skillScore` from each object before sending it
        const sanitizedData = data.map(({skillScore, ...restOfData}) => restOfData);
