...
export class GuestController {
	...
	controller = new Hono().post(
		...
		describeRoute({
   		 ...
   		 description: "Permite utilizatorilor neautentificați să creeze task-uri urgente (max 3 active).",
   		 tags: ["Guest Tasks"],
   		 responses: {
        		201: {
		...
		async (c) => {
			...
			if (!isValidUuid.success) {
				return sendApiResponse(c, null, { 
                    ...
                    message: "Format invalid pentru X-Guest-Session. Trebuie să fie UUID." 
                });
			}
			...
			try {
				...
				// 3. Creare task
				const result = await this.helpRequestService.createGuestHelpRequest(guestSession, body);
				
                return sendApiResponse(c, result, { kind: "created" }); // Returneaza 201

			} catch (error: any) {
				if (error instanceof ModerationError) {
					return sendApiResponse(c, null, { kind: "clientError", message: error.message });
				}
