...
export class VolunteerService {
  ...
  async getVolunteerProfile(userId: string) {
    ...
    const profile = await this.volunteerProfileRepo.findByVolunteerId(volunteer.id);
    return { volunteer, profile: profile ?? null };
  }

  async createVolunteerProfile(userId: string, data: {
    skills?: string[];
    maxDistanceKm?: number;
  }) {
    const volunteer = await this.getOrCreateVolunteer(userId);

    const existing = await this.volunteerProfileRepo.findByVolunteerId(volunteer.id);
  ...
  async updateVolunteerProfile(userId: string, data: {
    skills?: string[];
    maxDistanceKm?: number;
    ...
    const profile = await this.volunteerProfileRepo.findByVolunteerId(volunteer.id);
    if (!profile) throw new NotFoundError("VolunteerProfile", String(volunteer.id));
