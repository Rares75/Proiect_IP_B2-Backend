...
describe("HelpRequestService - getPaginatedOffersForTaskOwner", () => {
    ...
    test("includes name when hiddenIdentity is false", async () => {
        helpRequestRepo.findById.mockResolvedValueOnce({ requestedByUserId: "owner-123" });
        helpOfferRepo.findPaginatedOffersByTaskId.mockResolvedValueOnce({
            data: [makeOffer({ hiddenIdentity: false })],
            total: 1,
        });

        const result = await service.getPaginatedOffersForTaskOwner(1, "owner-123", 1, 10);
    ...
    test("computes totalPages correctly", async () => {
        ...
        const result = await service.getPaginatedOffersForTaskOwner(1, "owner-123", 2, 10);
    ...
    test("forwards status filter to repository", async () => {
        helpRequestRepo.findById.mockResolvedValueOnce({ requestedByUserId: "owner-123" });
        helpOfferRepo.findPaginatedOffersByTaskId.mockResolvedValueOnce({ data: [], total: 0 });

        await service.getPaginatedOffersForTaskOwner(1, "owner-123", 1, 10, "ACCEPTED");
