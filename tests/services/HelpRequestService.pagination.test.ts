...
import { RadiusRequiredError } from "../../src/services/helpRequestDistance";
...
describe("HelpRequestService.getPaginatedTasks distance defaults", () => {
	...
	it("throws RadiusRequiredError when user is not a volunteer and radius is missing", async () => {
		const service = new HelpRequestService(
			{
				findPaginatedWithDetails: async () => ({
					data: [],
		...
		).rejects.toBeInstanceOf(RadiusRequiredError);
	});
});
